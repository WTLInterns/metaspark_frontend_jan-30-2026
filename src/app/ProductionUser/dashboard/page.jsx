...
import { FiHome, FiInbox, FiClipboard, FiUsers, FiPackage, FiSettings, FiUserPlus, FiDollarSign, FiFileText, FiTruck, FiCheckCircle, FiChevronDown, FiChevronUp, FiMenu, FiX, FiSearch, FiBell, FiUser, FiLogOut } from 'react-icons/fi';
...
function handleLogout() {
  try {
    if (typeof window !== 'undefined') {
      ...
      document.cookie = 'swiftflow-auth=; path=/; max-age=0; samesite=strict';
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem('swiftflow-user');
      if (raw) {
  ...
  return (
    <div className="w-full">
      <div className="flex flex-col items-center justify-center w-full">
        ...
        <div className="flex items-center justify-between mb-6 w-full">
          ...
          <div className="relative flex items-center gap-3">
            ...
            <button
              ...
              className="h-8 w-8 rounded-full bg-gray-800 flex items-center justify-center text-xs font-semibold text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              {user?.name
                ? user.name
                    .split(' ')
            ...
            {showUserMenu && (
              <div className="absolute right-0 top-10 w-56 rounded-lg bg-white shadow-lg border border-gray-200 z-50 text-sm">
                <div className="px-4 py-3 border-b border-gray-100">
                  <p className="font-medium text-gray-900 text-sm">{user?.name || 'Admin User'}</p>
                  <p className="text-xs text-gray-500 truncate">{user?.email || 'admin@swiftflow.com'}</p>
                </div>
        ...
        <section className="bg-white border-t border-gray-200 p-6 w-full">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
            <div>
              <h2 className="text-lg font-semibold text-black">Production Department</h2>
              <p className="mt-1 text-sm text-gray-600">
                Oversee the production stage of orders.
              </p>
            </div>
            <div className="flex gap-3 w-full md:w-auto">
              ...
              <select className="border border-gray-200 rounded-md px-2 py-2 text-sm w-36 text-black">
                <option>All</option>
          ...
          <div className="border border-dashed border-gray-300 rounded-md py-16 flex flex-col items-center justify-center text-gray-400 text-sm">
