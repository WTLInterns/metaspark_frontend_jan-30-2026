...
import { getCustomerText, getProductText, formatOrderDate, getAssignedToText } from '@/utils/orderDisplay';
...
export default function ProductionLinePage() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const productionOrders = sorted.filter(order => {
      ...
      if (token) {
        ...
        try {
          ...
          if (assignedOrdersResponse.ok) {
            ...
            console.log('🔍 [PRODUCTION] Available filtered orders:', productionOrders.map(o => ({ id: o.orderId, name: o.product })));

            

            // Filter to only show orders assigned to this user

            const userOrders = productionOrders.filter(order => {
            ...
            const transformed = await Promise.all(userOrders.map(async (order) => {
              ...
              return {
                id: `SF${order.orderId}`,
                customer: getCustomerText(order),
                product: getProductText(order),
                quantity: order.units || '',
                status: order.status || 'Production',
                startDate: formatOrderDate(order.dateAdded),
                dueDate: '',
                assignedTo: getAssignedToText(order),
                machine: machineName,
